...
public class GraphicalCalculatorFrame extends JFrame
{
	...
	private final class GraphicalCalculatorPanel extends JPanel implements MouseListener
	{
		...
		private void setupRegions()
		{
			...
			this.regions = new Rectangle[5];
			
			regions[0] = new Rectangle(REGION_START_X, REGION_START_Y, REGION_WIDTH, REGION_HEIGHT);
			regions[1] = new Rectangle(REGION_START_X + 1*(REGION_INC_X), REGION_START_Y, REGION_WIDTH, REGION_HEIGHT);
			regions[2] = new Rectangle(REGION_START_X + 2*(REGION_INC_X), REGION_START_Y, REGION_WIDTH, REGION_HEIGHT);
			regions[3] = new Rectangle(REGION_START_X + 3*(REGION_INC_X), REGION_START_Y, REGION_WIDTH, REGION_HEIGHT);
			regions[4] = new Rectangle(REGION_START_X + 4*(REGION_INC_X), REGION_START_Y, REGION_WIDTH, REGION_HEIGHT);
		}
		...
		public void mouseClicked(MouseEvent e)
		{
			...
			for(int rect = 0; rect < regions.length; rect++)
			{
				if(regions[rect].contains(sourcePoint))
				{
					selectedRegion = rect;
				}
			}
		 ...
		 */
		public boolean setSelectedRegionContents(String content) // fix this with try and catch
		{
		...
		public int evaluate()
		{
			...
			String firstSign = operators[0];
			int secondVal = operands[1];
			String secondSign = operators[1];
			int thirdVal = operands[2];
			
			//used to evaluate the first two operands
			if(firstSign.equals("+"))
			{
			...
			//used to evaluate the last operand
			if(secondSign.equals("+"))
			{
				results += thirdVal;
			}
			else if(secondSign.equals("-"))
			{
				results -= thirdVal;
			}
			else
